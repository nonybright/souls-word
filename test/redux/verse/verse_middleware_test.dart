...
void main() {
  ...
  group('Verse middleware tests', () {
    ...
    test(
        'Saves category when saveCategoryaction is dispatched and calls successful when done',
        () async {
      when(categoryLocal.addCategory(typed(any))).thenAnswer((_) => null);
      when(categoryLocal.updateCategory(typed(any))).thenAnswer((_) => null);

      VerseCategory catWithId = VerseCategory(
          id: 1,
          name: 'cat',
          dateAdded: DateTime.now(),
          description: 'description');
      VerseCategory catWithOutId = VerseCategory(
          name: 'cat2', dateAdded: DateTime.now(), description: 'description');
      ...
      await verseMiddleWare.call(
          store, AddVerseCategory(catWithId, completer), next);
      verify(categoryLocal.updateCategory(typed(any)));
      expect(
          actions[1], const isInstanceOf<VerseCategoryAddSuccessfulAction>());

      actions.clear();

      final Completer<Null> completer2 = new Completer<Null>();
      await verseMiddleWare.call(
          store, AddVerseCategory(catWithOutId, completer2), next);
    ...
    test(
      ...
      when(verseRepository.getLatestVerses())
          .thenAnswer((_) => new Future.value(latestVerses));
    ...
    test(
        'Verse favorite toggled to a different state and changed in local repository',
        () async {
      Verse verse = Verse(
        id: 8,
        content: 'jesus wept 8',
      ...
      when(verseLocal.updateVerse(typed(any)))
      ...
      expect(actions[1],
          const isInstanceOf<ToggleVerseFavoriteSuccessFulAction>());
